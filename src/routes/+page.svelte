<script lang="ts">
	...
	export const snapshot = {
	  ...
	  restore: (value: { searchtext: string | AdvancedSearch; advancedToggle: boolean; advancedFields: Record<string, string> }) => {
	...
	const asyncSearch = async (query: string | AdvancedSearch, config: Record<string, unknown> = {}): Promise<Item[]> => {
	...
	<ContentContainer>
	  <div class="flex flex-col lg:flex-row gap-4 justify-between">
		...
		<div class="lg:w-1/2">
		  ...
		  <p class="mt-5">
			Found {searching ? '...' : filtereditems?.length} Result{filtereditems?.length !== 1 ? 's' : ''}.
		  </p>
		</div>
	  </div>
	</ContentContainer>
